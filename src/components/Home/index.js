...
class Home extends Component {
  ...
  onClickVisitBtn = id => {
    ...
    const visitCountry = listOfCountries.map(eachItem => {
      if (eachItem.id === id) {
  ...
  onClickRemoveBtn = id => {
    ...
    const removedCountry = listOfCountries.map(eachItem => {
      if (eachItem.id === id) {
        const removedItem = {
          ...eachItem,
          isVisited: !eachItem.isVisited,
  ...
  render() {
    const {listOfCountries} = this.state
    const updatedNewList = listOfCountries.filter(
    ...
    return (
      <div className="main-container">
        ...
        <ul className="countries-list-container">
          {listOfCountries.map(eachCountries => (
            <CountriesList
              key={eachCountries.id}
              countryDetails={eachCountries}
        ...
        <ul className="visited-container">
          {updatedNewList.length === 0 ? (
            <p className="not-visited">No Countries Visited Yet!</p>
          ) : (
            updatedNewList.map(each => (
